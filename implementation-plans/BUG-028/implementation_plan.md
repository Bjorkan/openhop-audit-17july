...
[← Finding](../../findings/BUG-028-wsradio-cannot-be-used-with-the-dispatcher.md) · [← Audit index](../../README.md)
...
| Field | Value |
|---|---|
| Finding | **WsRadio cannot be used with Dispatcher or MeshNode** |
...
|---|---|---|
| OpenHop Core | `src/openhop_core/hardware/wsradio.py` | Confirmed evidence and primary change surface |
...
- [ ] Confirm whether `WsRadio` is still a supported public adapter or should be removed/deprecated.
- [ ] Verify only one coroutine owns `websocket.recv()`; `_send_radio_config()` currently reads an acknowledgement directly.
...
- [ ] Preserve verification method 1: static contract trace — `WsRadio` has no `set_rx_callback`, while `Dispatcher.__init__` calls it unconditionally.
- [ ] Preserve verification method 2: dispatcher construction — Creating `Dispatcher(WsRadio(...))` raises `AttributeError`.
- [ ] Preserve verification method 3: public node path — Creating `MeshNode` with the same radio fails at the same reachable constructor path.
