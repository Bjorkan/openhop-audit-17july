# POSSIBLE-ENHANCEMENT-008 — implementation plan

[← Finding](../../findings/POSSIBLE-ENHANCEMENT-008-write-configuration-atomically-and-keep-a-last-known-good-copy.md) · [← Audit index](../../README.md)
...
| Field | Value |
|---|---|
| Finding | **Possible enhancement — write configuration atomically and keep a last-known-good copy** |
...
Implement the maintainability improvement without changing unrelated behavior: Serialize and fsync a temporary file in the same directory, validate it, then `os.replace()` it; retain a bounded `.bak` copy.
...
|---|---|---|
| OpenHop Repeater | `repeater/config_manager.py` | Evidence lines 227–252 |
...
1. Write validated YAML to a same-filesystem temporary file, flush and fsync it, then atomically replace the active file.
2. Maintain a last-known-good backup and validate it before automatic recovery.
3. Preserve file permissions/ownership and safely clean stale temporary files.
4. Test interruption, disk-full, invalid YAML and recovery paths.
