...
[← Finding](../../findings/BUG-037-command-responses-use-one-global-unkeyed-slot.md) · [← Audit index](../../README.md)
...
| Field | Value |
|---|---|
| Finding | **Companion command responses use one global unkeyed callback slot** |
...
| OpenHop Core | `src/openhop_core/node/handlers/text.py` | Confirmed evidence and primary change surface |
...
- [ ] Confirm the on-air protocol has no explicit command-response marker; if not, define safe correlation rules and concurrency limits.
- [ ] Preserve ordinary `TXT_TYPE_PLAIN` delivery from the same contact when it is not a command response.
...
- [ ] Preserve verification method 1: static unkeyed slot — One callback is installed and receive handling has no expected sender/request check.
- [ ] Preserve verification method 2: unrelated message — An ordinary plain message satisfies the callback and disappears from normal delivery.
- [ ] Preserve verification method 3: overlapping public calls — A response labeled from A completes B while A remains pending.
