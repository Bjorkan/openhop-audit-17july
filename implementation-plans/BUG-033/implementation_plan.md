...
[← Finding](../../findings/BUG-033-kiss-connection-state-remains-healthy-after-worker-or-config-failure.md) · [← Audit index](../../README.md)
...
| Field | Value |
|---|---|
| Finding | **KISS connection state remains healthy after initialization or worker failure** |
...
- [ ] Coordinate RX/TX worker failure so only one cleanup/reconnect owner runs.
- [ ] Avoid joining the current worker thread from its own failure path.
...
- [ ] Preserve verification method 1: auto-config failure — `connect()` returns false while connected state, open serial and started threads remain.
- [ ] Preserve verification method 2: tx-worker failure — The worker exits but `is_connected()` stays true and accepts another queued frame.
- [ ] Preserve verification method 3: context manager — `with radio:` enters despite `connect()` returning false.
