...
[← Finding](../../findings/BUG-035-concurrent-radio-commands-with-the-same-response-type-collide.md) · [← Audit index](../../README.md)
...
| Field | Value |
|---|---|
| Finding | **Concurrent radio commands expecting the same response type overwrite one another** |
...
|---|---|---|
| OpenHop Core | `src/openhop_core/hardware/tcp_radio.py` | Confirmed evidence and primary change surface |
...
- [ ] Confirm whether unsolicited responses can share these command bytes.
- [ ] Keep TX_DONE synchronization separate from configuration-command serialization.

## Test plan

### Existing test modules likely to extend

- OpenHop Core: `tests/test_companion_radio.py`
- OpenHop Core: `tests/test_radio_capabilities.py`

### Required focused tests

- [ ] Preserve verification method 1: static correlation key — Both adapters index waiters only by response command and have no command-level serialization.
- [ ] Preserve verification method 2: tcp concurrency — Two commands are sent; one returns data and the other times out.
- [ ] Preserve verification method 3: usb concurrency — The same collision reproduces independently in the USB adapter.
