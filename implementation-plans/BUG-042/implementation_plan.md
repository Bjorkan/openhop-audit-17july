...
[← Finding](../../findings/BUG-042-rrd-counter-rates-are-treated-as-cumulative-counters.md) · [← Audit index](../../README.md)
...
| Field | Value |
|---|---|
| Finding | **RRD COUNTER rates are treated as cumulative counter values** |
...
|---|---|---|
| OpenHop Repeater | `repeater/data_acquisition/rrdtool_handler.py` | Confirmed evidence and primary change surface |
...
- [ ] Confirm exact `rrdtool.fetch` consolidation function and unknown-value handling.
- [ ] Prevent double integration if any current source is already a per-interval count.
...
- [ ] Preserve verification method 1: static datasource/reader contradiction — RRD defines `COUNTER`; both readers compute differences between fetched values.
- [ ] Preserve verification method 2: packet-type reader — Ten constant non-zero rate samples produce total zero.
- [ ] Preserve verification method 3: dashboard buckets — Constant RX/TX rates produce zero counts in every bucket.
