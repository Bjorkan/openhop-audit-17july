...
| Executable reproduction | **Passed** | `test_bug_004_confirmed_live_radio_mutation_does_not_update_airtime_parameters` in [`REVERIFICATION-CHECKS.py`](../docs/REVERIFICATION-CHECKS.py). |
| Active falsification | **Passed** | Airtime calculation remains bound to cached modulation fields and live radio apply does not synchronize them. See [`BASELINE-FALSIFICATION-CHECKS.py`](../docs/BASELINE-FALSIFICATION-CHECKS.py). |
