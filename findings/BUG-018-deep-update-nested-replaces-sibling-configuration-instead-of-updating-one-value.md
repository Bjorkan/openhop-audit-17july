...
| Executable reproduction | **Passed** | `test_bug_018_confirmed_update_nested_clobbers_siblings_but_has_no_repo_call_sites` in [`REVERIFICATION-CHECKS.py`](../docs/REVERIFICATION-CHECKS.py). |
| Active falsification | **Passed** | No recursive merge protects siblings; no supplied call site exists, preserving the latent classification. See [`BASELINE-FALSIFICATION-CHECKS.py`](../docs/BASELINE-FALSIFICATION-CHECKS.py). |
