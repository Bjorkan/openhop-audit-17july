# BUG-016 — The raw duplicate path bypasses the configured per-packet duplicate cap
...
Normal duplicate grouping limits each packet to 20 nested duplicate records, but `record_duplicate()` appends without that limit.
...
Every entry path that groups duplicates under an original must apply the same cap and accounting semantics.
...
| Executable reproduction | **Passed** | `test_bug_016_confirmed_raw_duplicate_path_bypasses_cap` in [`REVERIFICATION-CHECKS.py`](../docs/REVERIFICATION-CHECKS.py). |
| Active falsification | **Passed** | The normal duplicate path enforces the configured cap; the raw path appends without it. See [`BASELINE-FALSIFICATION-CHECKS.py`](../docs/BASELINE-FALSIFICATION-CHECKS.py). |
...
### Evidence 2: `repeater/engine.py` lines 568–583
...
### Evidence 3: `repeater/engine.py` lines 655–732
