...
| Executable reproduction | **Passed** | `test_bug_007_confirmed_advert_endpoint_claims_immediate_application_after_failure` in [`REVERIFICATION-CHECKS.py`](../docs/REVERIFICATION-CHECKS.py). |
| Active falsification | **Passed** | The endpoint wraps a success response, forces `restart_required=false`, and has no failed-save guard. See [`BASELINE-FALSIFICATION-CHECKS.py`](../docs/BASELINE-FALSIFICATION-CHECKS.py). |
...
[Open `implementation_plan.md`](../implementation-plans/BUG-007/implementation_plan.md)
