...
| Executable reproduction | **Passed** | `test_bug_017_confirmed_reload_default_differs_and_drops_missing_cache_ttl_to_60` in [`REVERIFICATION-CHECKS.py`](../docs/REVERIFICATION-CHECKS.py). |
| Active falsification | **Passed** | Startup and reload still use different defaults, and only startup applies the lower bound. See [`BASELINE-FALSIFICATION-CHECKS.py`](../docs/BASELINE-FALSIFICATION-CHECKS.py). |
