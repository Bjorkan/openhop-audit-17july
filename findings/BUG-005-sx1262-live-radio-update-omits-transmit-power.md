# BUG-005 — SX1262 live radio updates omit transmit power
...
When a radio exposes `configure_radio`, the live-update path applies frequency, bandwidth, SF and CR but never calls `set_tx_power`. The endpoint can still report that all settings were applied immediately.
...
The missing live setter call is proven in source and with a deterministic radio adapter. No physical SX1262 transmission-power measurement was performed, so the on-air effect remains hardware-dependent.

## Expected behaviour / proposed direction

Changing TX power must either call the hardware setter and verify success or be reported as requiring restart.
...
Focused fake-SX1262 check recorded a successful `configure_radio` call, zero `set_tx_power` calls, and unchanged runtime power.
...
| Executable reproduction | **Passed** | `test_bug_005_confirmed_configure_radio_branch_omits_tx_power` in [`REVERIFICATION-CHECKS.py`](../docs/REVERIFICATION-CHECKS.py). |
| Active falsification | **Passed** | The `configure_radio` capability branch has no fallback call to the separate TX-power setter. See [`BASELINE-FALSIFICATION-CHECKS.py`](../docs/BASELINE-FALSIFICATION-CHECKS.py). |
...
[Open `implementation_plan.md`](../implementation-plans/BUG-005/implementation_plan.md)
...
### Evidence 2: `src/openhop_core/hardware/sx1262_wrapper.py` lines 1589–1598
...
### Evidence 3: `src/openhop_core/hardware/sx1262_wrapper.py` lines 1622–1684
